...
def int_to_uuid(int_value: int) -> uuid.UUID:
    """
    Used to create a uuid from an int.
    """
...
def save_chat(event, logger):
    """
    Function responsible to creating a new chat.
    """
    try:
...
def patch_chat(event, logger):
    """
    Function responsible to sent a patch if the chat has been modified.
    Its important to construct appropirte dictionaries
    """
    try:
        ...
        if event.get("data") and event.get("data", {}).get("new_advisor_id"):
            advisor = requests.get(
                f"{BIG_CHAT_API}/advisors/{event.get('data', {}).get('new_advisor_id')}")  # We need advisor email
            parameters = {"email": advisor.json()["email_address"]}
            agent = requests.get(f"{OUR_API}/agents", params=parameters)  # We need agent uuid
            if len(agent.json()) == 0:
...
def save_agent(event, logger):
    """
    Function responsible to creating a new agent.
    """
    try:
...
def save_message(event, logger):
    """
    Function to save a message of a chat.
    """
    try:
        ...
        parameters = {"external_id": int_to_uuid(event["conversation_id"])}
        chat = requests.get(f"{OUR_API}/chats", params=parameters)  # To check if the chat of the message already exists

        # Save the chat ig doesnt exist.
